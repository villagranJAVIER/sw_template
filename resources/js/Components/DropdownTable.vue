    ...
    <div class="relative inline-block text-left dropdown-menu" @mouseenter="handleMouseEnter"
        ...
        <div class="max-w-xs">
            <button type="button" :class="itemClass"
                class="inline-flex w-full justify-center rounded-md bg-white dark:bg-gray-700 font-bold text-gray-900 dark:text-white ring-inset ring-gray-300 hover:bg-gray-100">
                {{ title }}
        ...
        <div v-show="show"
            ...
            <div v-for="option in fp.menu" :key="option">
                <span :class="itemClass"
                    class="text-gray-700 dark:text-white block text-sm hover:bg-gray-100 hover:dark:bg-slate-600 cursor-pointer"
                    @click="$emit('sortByColumn', fp.key, option.direction)">
                    {{ option.title }}
                </span>
            </div>
        </div>
